...
void FrontSlam::execute(Boss& boss, Player& player) {
    ...
    // boss.playAnimation("front_slam_windup"); // 준비 애니메이션 재생
}
...
void FrontSlam::update(float dt, Boss& boss, Player& player) {
    ...
    // 1. 선딜레이(Wind-up) 구간
    if (currentCooldown < WIND_UP_DURATION) {
        // 아직 공격 판정 없음
        isHitboxActive_ = false;
    }
    // 2. 공격(Active) 구간
    else if (currentCooldown < WIND_UP_DURATION + ACTIVE_DURATION) {
        // 공격 판정 활성화!
    ...
    // 3. 후딜레이(Recovery) 및 패턴 종료
        ...
        if (patternTimer_ >= WIND_UP_DURATION + ACTIVE_DURATION + RECOVERY_DURATION) {
